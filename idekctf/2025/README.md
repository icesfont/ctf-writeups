...
- I feared people might try to lag the server instead in order to hold back the request, which FluxFingers did, so I was considering making the challenge instanced with one sync worker, but I also feared that this might lead to too many deep rabbit holes.
